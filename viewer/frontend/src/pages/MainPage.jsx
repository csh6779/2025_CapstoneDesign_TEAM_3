...
function MainPage() {
    ...
    // ✅ 팀원 수정: '사진 관리', '내 이미지', '북마크' 섹션 토글 State 추가
    ...
    useEffect(() => {
        ...
        if (storedLoginId) {
            ...
            // ✅ 팀원 수정: 다른 페이지에서 전달된 데이터셋이 있는지 확인 및 로드
            if (location.state && location.state.selectedDatasets && location.state.selectedDatasets.length > 0) {
                const passedDatasets = location.state.selectedDatasets;
                console.log("📦 FileSelectPage에서 배달 온 데이터:", passedDatasets);

                // 1. 넘어온 데이터들을 내 목록(volumes)에 추가
                setVolumes(passedDatasets);

                // 2. 첫 번째 데이터를 바로 뷰어에 띄우기
                setCurrentVolume(passedDatasets[0]);

                // 3. 토스트 메시지
                showToast(`🚀 ${passedDatasets[0].name} 로드됨`);

                // URL state 청소 (새로고침 시 중복 방지)
                window.history.replaceState({}, document.title);
            } else {
                // 전달된 데이터가 없으면 서버에서 목록 조회
                console.log("📭 배달 온 데이터 없음. 서버에서 목록 조회 시작.");

                const loadInitialVolumes = async () => {
                   try {
                        const token = localStorage.getItem('accessToken');
                        if (!token) return;
                        const response = await fetch(`${API_BASE_URL}/api/volumes?LoginId=${storedLoginId}`, {
    ...
    // --- 볼륨 관리 함수 ---
    const loadVolumes = async (LoginId = user.LoginId) => {
        try {
            const token = localStorage.getItem('accessToken');
            if (!token) return;

            const response = await fetch(`${API_BASE_URL}/api/volumes?LoginId=${LoginId}`, {
                headers: {
    ...
    const handleDeleteVolume = async (volumeName, event) => {
        ...
        try {
            ...
            const response = await fetch(
                `${API_BASE_URL}/api/volumes/${volumeName}?LoginId=${user.LoginId}`,
                {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`
    ...
    const handleSubmitNewPassword = async () => {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/v1/users/${user.id}`, { // 참고: user.id가 없으면 백엔드 확인 필요
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    const getNeuroglancerUrl = (volume) => {
        ...
        const locationPath = volume.location || 'docker';
        const sourceUrl = `precomputed://${API_BASE_URL}/precomputed/${locationPath}/${volume.name}`;

        console.log("생성된 Neuroglancer Source URL:", sourceUrl);
